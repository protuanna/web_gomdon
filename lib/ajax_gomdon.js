...
export async function bill(data) {
    ...
    if (session !== null) {
        ...
        let result = await Axios({
            method: 'get',
            url: process.env.NEXT_PUBLIC_GOMDON_API_URI + '/api/v2/bill',
            /*params: { type: type, keyword:search, page:page },*/
            params: data,
            headers: {Authorization: `Bearer ` + token},
        }).then(function (response) {
...
export async function search_address(data) {
    let result = await Axios({
        ...
        return error.response.data ?? {result: false, qessage: "Lấy dữ liệu k thành công"}
    });
    return result;
}
...
export async function sendMessage(message) {
    ...
    if (session !== null) {
        ...
        let result = await Axios({
            method: 'post',
            url: process.env.NEXT_PUBLIC_GOMDON_CHAT_URI + '/include/api.php',
            data: bodyFormData
        }).then(function (response) {
            console.log(response)
            return response.data;
        }).catch(function (error) {
            return error.response.data ?? {result: false, message: "Gửi tin nhắn không thành công"}
        });
        return result;
    }
    return {result: false, message: "Vui lòng đăng nhập"}
