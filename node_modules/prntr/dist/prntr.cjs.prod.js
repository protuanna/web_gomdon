...
var browser = typeof window !== 'undefined' ? Bowser__default["default"].getParser(window.navigator.userAgent) : undefined;
var browserName = browser === null || browser === void 0 ? void 0 : browser.getBrowserName();
var platform = browser === null || browser === void 0 ? void 0 : browser.getPlatformType();
var engine = browser === null || browser === void 0 ? void 0 : browser.getEngine().name;
var Browser = {
  /* Firefox */
  ...
  /* ipad */
  isIpad: typeof window !== 'undefined' && navigator.maxTouchPoints && navigator.maxTouchPoints > 2 && navigator.platform.includes('MacIntel'),
...
function removeIframe(frameId) {
  ...
  setTimeout(function () {
    return iframe === null || iframe === void 0 ? void 0 : iframe.remove();
  }, delay);
}

function cleanupFast(config) {
  var onLoadingStart = config.onLoadingStart,
      onLoadingEnd = config.onLoadingEnd,
      printable = config.printable,
      frameId = config.frameId,
      iframeRemovalDelay = config.iframeRemovalDelay; // Check for a finished loading hook function

  onLoadingEnd === null || onLoadingEnd === void 0 ? void 0 : onLoadingEnd(); // If preloading pdf files, clean blob url

  if (onLoadingStart && typeof printable === 'string') window.URL.revokeObjectURL(printable); // Remove Iframe
...
function cleanUp(config) {
  ...
  if (Browser$1.isChrome || Browser$1.isEdge && !Browser$1.isEdgeHTML || Browser$1.isFirefox) {
  ...
  var handler = function handler() {
    ...
    window.removeEventListener(event, handler); // Run onPrintDialogClose callback
...
function print(config, printFrame, printableElement) {
  ...
  iframeElement.onload = function () {
    ...
    var printDocument = getPrintDocument(iframeElement);
    ...
    addStyle(style, printDocument); // If printing images, wait for them to load inside the iframe
...
function performPrint(iframeElement, config) {
  ...
  try {
    ...
    if (Browser$1.isEdgeHTML || Browser$1.isIE) {
      try {
        var _iframeElement$conten;

        (_iframeElement$conten = iframeElement.contentWindow) === null || _iframeElement$conten === void 0 ? void 0 : _iframeElement$conten.document.execCommand('print', true);
      } catch (e) {
...
function addStyle(style, printDocument) {
  if (!style) return; // Create style element
...
function getPrintDocument(iframe) {
  var printDocumentWindow = iframe.contentWindow || iframe.contentDocument;
  var printDocument = printDocumentWindow && 'document' in printDocumentWindow ? printDocumentWindow.document : null;
...
function cloneElement(element, config) {
  ...
  var _config$ignoreElement = config.ignoreElements,
      ...
      shouldScanStyles = config.scanStyles; // Clone the main node (if not already inside the recursion process)
...
function image(config, printFrame) {
  ...
  appendImages(images, printableElement); // Check if we are adding a print header
...
function appendImages(images, printableElement) {
...
function json(config, printFrame) {
  var printable = config.printable,
      shouldRepeatTableHeader = config.repeatTableHeader,
  ...
  var mappedConfig = getConfigWithMappedProperties(config, properties); // Create a print container element
...
function getConfigWithMappedProperties(config, properties) {
...
function pdf(config, printFrame) {
  var hasBase64 = config.base64,
      printable = config.printable; // Check if we have base64 data

  if (hasBase64) {
    var bytesArray = Uint8Array.from(window.atob(printable), function (c) {
      return c.charCodeAt(0);
    });
    createBlobAndPrint$1(config, printFrame, bytesArray);
    return;
...
function createPdfRequest(config, printFrame) {
  var printable = config.printable,
      onError = config.onError; // Format pdf url

  var pdfUrl = getPdfUrl(printable); // Request pdf url
  ...
  req.addEventListener('load', function () {
    ...
    createBlobAndPrint$1(config, printFrame, req.response);
  });
...
function getPdfUrl(printable) {
  return /^(blob|http|\/\/)/i.test(printable) ? printable : window.location.origin + (!printable.startsWith('/') ? '/' + printable : printable);
}

function createBlobAndPrint$1(config, printFrame, data) {
...
var printTypes = ['pdf', 'html', 'image', 'json', 'raw-html']; // Define default configs
...
function start(config) {
  ...
  switch (config.type) {
    case 'pdf':
      startPdf(_objectSpread2(_objectSpread2({}, baseIPdfConfig), config), printFrame);
      break;
    ...
    case 'html':
      startOther(config, html(_objectSpread2(_objectSpread2({}, baseIHtmlConfig), config), printFrame));
      break;
    ...
    case 'json':
      startOther(config, json(_objectSpread2(_objectSpread2({}, baseJsonConfig), config), printFrame));
      break;
  }
}

function createBlobAndPrint(data) {
  // Pass response or base64 data to a blob and create a local object url
  var localPdfBlb = new window.Blob([data], {
    type: 'application/pdf'
  });
...
function getBlob(printable) {
  return Uint8Array.from(window.atob(printable), function (c) {
    return c.charCodeAt(0);
  });
}
...
function printFallback(config, hasHref) {
  ...
  var win = window.open(fallbackPrintable, '_blank', 'toolbar=yes,scrollbars=yes,menubar=no,location=no,resizable=yes');
  win === null || win === void 0 ? void 0 : win.focus();
}

function startOther(config, callback) {
  var onLoadingEnd = config.onLoadingEnd,
      onIncompatibleBrowser = config.onIncompatibleBrowser,
      onError = config.onError,
      frameId = config.frameId; // Check browser support for pdf and if not supported we will just open the pdf file instead

  if (Browser$1.isChromeMobile || Browser$1.isIosChrome) {
    try {
...
function startPdf(config, printFrame) {
  ...
  if (Browser$1.isIE || Browser$1.isSafari || Browser$1.isChromeMobile || Browser$1.isIosChrome) {
    try {
      if (Browser$1.isIE) console.info('Prntr doesn\'t support PDF printing in Internet Explorer');
      if (Browser$1.isChromeMobile || Browser$1.isIosChrome) console.info('Prntr doesn\'t support PDF printing in Chrome on Mobile');
      if (Browser$1.isSafari && !config.base64) console.info('Prntr doesn\'t support PDF printing in Safari');
      if (Browser$1.isSafari && !Browser$1.isIpad && config.base64) console.info('Prntr doesn\'t support PDF Base64 printing in Safari on Desktop or Iphone');
      var hasHref = !!Browser$1.isChromeMobile || config.base64;
